...
plot_data <- plot_data %>%
  ...
  mutate(Yearly_Total = sum(Visitors), Percent = (Visitors/Yearly_Total)*100, #Parkwide = sum(Percent)
         )
...
joined_data <- joined_data %>%
  #rename(RecVisitors_Parkwide = RecreationVisitors.y, RecVisitors_byPark = RecreationVisitors.x) %>%
  #select(-UnitName.y) %>%
  group_by(Year) %>%
  filter(UnitCode.x != "Parkwide") %>%
  mutate(Yearly_Total = sum(RecVisitors_byPark), Percent = (RecVisitors_byPark/Yearly_Total)*100, #Parkwide = sum(Percent)
         )
...
plot_data <- plot_data %>%
  filter(Year >= 1990) %>%
  group_by(Park) %>%
  summarize(visit_mean = mean(Visitors))
  
#When averaging across years, GRSM has the highest number of visitors with 10,156,490 on average and ACAD has the fewest visitors with 2,747,780 on average between 1990 and 2024. 

arrange(plot_data, visit_mean)
print(plot_data, n=8)
