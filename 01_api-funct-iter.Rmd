...
To utilize the NPS API in R, we first need to explore its [API's data structure](https://irmaservices.nps.gov/). In almost every case, we use URLs to access specific data from APIs. To find the access URL for NPS visitation data, go to [Stats Rest API - Documentation](https://irmaservices.nps.gov/v3/rest/stats/help) (though not very intuitive, the NPS API calls its visitation data set "Stats"). Listed there you will see that all data associated with the "Stats" data set can be accessed using the base URL **https://irmaservices.nps.gov/v3/rest/stats**. From there, you can tack on additional html text to access two different data sets: **total/{year}** and **visitation**.
...
data_1980 <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1980")
unpacked_1980 <- httr::content(data_1980, as = "text", encoding = "UTF-8") 
final1980 <- jsonlite::fromJSON(unpacked_1980)

data_1999 <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1999")
unpacked_1999 <- httr::content(data_1999, as = "text", encoding = "UTF-8") 
final1999 <- jsonlite::fromJSON(unpacked_1999)

data_2018 <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/2018")
unpacked_2018 <- httr::content(data_2018, as = "text", encoding = "UTF-8") 
final2018 <- jsonlite::fromJSON(unpacked_2018)

final_output <- bind_rows(final1980, final1999, final2018)
...
```{r}
base_url_visits <- "https://irmaservices.nps.gov/v3/rest/stats/visitation"
query_params <- list(
  unitCode = "ROMO",
  startMonth = 1,
  startYear = 2010,
...
ROMO_rawdata <- GET(base_url_visits, query = query_params)
...
# pull in the data
url <- paste0("https://irmaservices.nps.gov/v3/rest/stats/total/", year)
...
```{r}
unit_visitation <- function(unitCode,
  startMonth,
  startYear,
  endMonth,
  endYear) {
  url_unitvis <- paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?", "unitCode=", unitCode,
    "&startMonth=", startMonth,
    "&startYear=", startYear,
    "&endMonth=", endMonth,
    "&endYear=", endYear)
  raw_data <- GET(url_unitvis)
extracted_data <- content(raw_data, as = "text", encoding = "UTF-8")
  
  # Parse JSON to data frame
  final_data <- fromJSON(extracted_data)
...
```{r}
pull_ROMO9224 <- unit_visitation("ROMO", 1, 1992, 12, 2024)

pull_EVER9224 <- unit_visitation("EVER", 1, 1992, 12, 2024)

pull_THRO9224 <- unit_visitation("THRO", 1, 1992, 12, 2024)
```
...
```{r}
park_codes <- c("ROMO", "EVER", "THRO")
visitation <- park_codes %>% 
  map(~ unit_visitation(code = .x, startyear = 1980, endyear = 2024) %>% 
        mutate(UnitCode = .x)) %>%  # Add a column to identify the park
  bind_rows()
