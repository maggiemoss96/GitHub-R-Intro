...
poudre_points_elev <- terra::extract(co_elevation_crop, poudre_points_prj) %>%
      rename(elevation_m = fileedcdb805338)
poudre_points_elev
...
```{r}
pa_elevation <- get_elev_raster(pa_counties, z = 7)
```
...
#tm_borders works best here because it allows the other spatial layers to be seen through the transparent county shapes. tm_polygons would obscure the underlying data layers. 
```
...
```{r}
schuylkill <- pa_counties %>%
  filter(NAMELSAD == "Schuylkill County")

schuylkill_terra <- vect(schuylkill)
#crs(pa_elevation_terra)
#crs(schuylkill_terra)
schuylkill_terra <- project(schuylkill_terra, pa_elevation_terra)

pa_elevation_terra <- rast(pa_elevation) 
class(pa_elevation)
pa_elevation_crop <- crop(pa_elevation_terra, schuylkill_terra)

#map it
tm_shape(pa_elevation_crop) +
  tm_raster(col.scale = tm_scale_continuous()) # make it continuous color scale
```
